# code from
# github.com/tensorflow/models/blob/master/research/differential_privacy/privacy_accountant/python/gaussian_moments.py
...
def distributions(sigma, q):
    def mu0(y):
        return pdf_gauss(y, sigma=sigma, mean=0.0)

    def mu1(y):
        return pdf_gauss(y, sigma=sigma, mean=1.0)

    def mu(y):
        return (1 - q) * mu0(y) + q * mu1(y)
    return mu0, mu1, mu
...
def compute_a(sigma, q, lmbd, verbose=False):
    ...
    if verbose:
        print("A: by binomial expansion    {} = {} + {}".format(a_lambda_exact, (1.0 - q) * a_lambda_first_term_exact,
...
def compute_b(sigma, q, lmbd, verbose=False):
    ...
    def b_fn(z): return np.power(mu0(z) / mu(z), lmbd) - np.power(mu(-z) / mu0(z), lmbd)
    ...
    def b_lambda_int1_fn(z): return mu0(z) * np.power(cropped_ratio(mu0(z), mu(z)), lmbd)

    def b_lambda_int2_fn(z): return mu0(z) * np.power(cropped_ratio(mu(z), mu0(z)), lmbd)
...
def pdf_gauss_mp(x, sigma, mean):
    return mp.mpf(1.) / mp.sqrt(mp.mpf("2.") * sigma ** 2 * mp.pi) * \
           mp.exp(- (x - mean) ** 2 / (mp.mpf("2.") * sigma ** 2))
...
def compute_a_mp(sigma, q, lmbd, verbose=False):
    ...
    if verbose:
        ...
        print("A: by numerical integration {} = {} + {}".format(a_lambda, (1 - q) * a_lambda_first_term,
...
def compute_b_mp(sigma, q, lmbd, verbose=False):
    ...
    if verbose:
        def b_fn(z): return (mu0(z) / mu(z)) ** lmbd_int - (mu(-z) / mu0(z)) ** lmbd_int
        print("M =", m)
        print("f(-M) = {} f(M) = {}".format(b_fn(-m), b_fn(m)))
        assert b_fn(-m) < 0 and b_fn(m) < 0
...
def _compute_eps(log_moments, delta):
    ...
    for moment_order, log_moment in log_moments:
        ...
        # if delta <= 0.:
        #     continue
        # print((log_moment - math.log(delta)) / moment_order, - math.log(delta), log_moment)
